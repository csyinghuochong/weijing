...
namespace ET
{
    ...
    [ActorMessageHandler]
    public class C2C_ChatSendHandler : AMActorRpcHandler<Scene, C2C_SendChatRequest, C2C_SendChatResponse>
    {

        protected override async ETTask Run(Scene scene, C2C_SendChatRequest request, C2C_SendChatResponse response, Action reply)
        {
            ...
            switch (request.ChatInfo.ChannelId)
            {
                case (int)ChannelEnum.Word:
                    for (int i = 0; i < chatComponent.WordActorList.Count; i++)
                    {
                        ...
                        MessageHelper.SendActor(chatComponent.WordActorList[i].GateSessionActorId, m2C_SyncChatInfo);
                    }
                    break;
                case (int)ChannelEnum.Friend:
                    ...
                    g2M_UpdateUnitResponse = (G2T_GateUnitInfoResponse)await ActorMessageSenderComponent.Instance.Call
                          (gateServerId, new T2G_GateUnitInfoRequest()
                          {
                              UserID = request.ChatInfo.PlayerId
                          });
                    MessageHelper.SendActor(g2M_UpdateUnitResponse.SessionInstanceId, m2C_SyncChatInfo);
                    break;
                case (int)ChannelEnum.Team:
                    ...
                    T2C_GetTeamInfoResponse g_SendChatRequest1 = (T2C_GetTeamInfoResponse)await ActorMessageSenderComponent.Instance.Call
                        (teamServerId, new C2T_GetTeamInfoRequest() { UserID = request.ChatInfo.PlayerId });

                    gateServerId = StartSceneConfigCategory.Instance.GetBySceneName(scene.DomainZone(), "Gate1").InstanceId;
