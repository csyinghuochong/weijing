...
namespace ET
{
    ...
    public class M2E_EMailReceiveHandler : AMActorRpcHandler<Scene, M2E_EMailReceiveRequest, E2M_EMailReceiveResponse>
    {
        ...
        protected override async ETTask Run(Scene scene, M2E_EMailReceiveRequest request, E2M_EMailReceiveResponse response, Action reply)
        {
            ...
            D2G_GetComponent d2GGetUnit = (D2G_GetComponent)await ActorMessageSenderComponent.Instance.Call(dbCacheId, new G2D_GetComponent() { CharacterId = request.Id, Component = DBHelper.DBMailInfo });
            ...
            D2M_SaveComponent d2GSave = (D2M_SaveComponent)await ActorMessageSenderComponent.Instance.Call(dbCacheId, new M2D_SaveComponent() { CharacterId = request.Id, Component = dBMailInfo, ComponentType = DBHelper.DBMailInfo });
            reply();
        }
    }
}
