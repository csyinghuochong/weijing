                        ...
                        switch (itemConfig.ItemSubType)
                        {
                            ...
                            case 4:
                                if (unit.DomainScene().GetComponent<MapComponent>().SceneTypeEnum == (int)SceneTypeEnum.LocalDungeon)
                                {
                                    ...
                                    TransferHelper.LocalDungeonTransfer_Old(unit, 0, int.Parse(itemConfig.ItemUsePar), localDungeon.FubenDifficulty).Coroutine();
                                }
                                break;
                                //图纸制造
                            case 5:
                            ...
                            case 6:
                                ...
                                unit.GetComponent<BagComponent>().OnAddItemData(rewardItems, string.Empty, $"{ItemGetWay.ItemBox_6}_{TimeHelper.ServerNow()}_{itemConfig.Id}");
                                break;
                            //兑换：
                            case 8:
                                string[] duihuanparams = itemConfig.ItemUsePar.Split(';');
