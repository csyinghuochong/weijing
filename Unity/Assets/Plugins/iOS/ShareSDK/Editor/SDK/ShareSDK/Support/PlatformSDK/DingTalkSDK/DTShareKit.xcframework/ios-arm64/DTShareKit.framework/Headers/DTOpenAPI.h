...
/**
 * @brief 打开钉钉客户端.
 * @return 仅为了兼容老接口，固定返回 YES，无实际意义，请使用 openDingTalkOnCompletion
 */
+ (BOOL)openDingTalk __deprecated_msg("Use openDingTalkOnCompletion");

/**
 * @brief 打开钉钉客户端.
 * @param completion 打开结果回调。success 为 YES 则成功打开钉钉客户端，NO 未能打开钉钉客户端.
 */
+ (void)openDingTalkOnCompletion:(void (^)(BOOL success))completion;

/**
 * @brief 打开支持 SSO 授权的钉钉客户端
 * @return 仅为了兼容老接口，固定返回 YES，无实际意义，请使用 openDingTalkOnCompletion
 */
+ (BOOL)openDingTalkForSSO __deprecated_msg("Use openDingTalkForSSOOnCompletion");

/**
 * @brief 打开支持 SSO 授权的钉钉客户端
 * @param completion 打开结果回调。success 为 YES 则成功打开钉钉客户端，NO 未能打开钉钉客户端.
 */
+ (void)openDingTalkForSSOOnCompletion:(void (^)(BOOL success))completion;


/**
 * @brief 发送请求到钉钉, 钉钉处理完请求后会回调第三方APP, 第三方APP需在 -[DTOpenAPIDelegate onReq:] 处理钉钉的响应结果.
 * @param req 封装请求的对象. 支持的类型: DTSendMessageToDingTalkReq.
 * @return 仅为了兼容老接口，固定返回 YES，无实际意义，请使用 sendReq:completion
 */
+ (BOOL)sendReq:(DTBaseReq *)req __deprecated_msg("Use sendReq:completion:");

/**
 * @brief 发送请求到钉钉, 钉钉处理完请求后会回调第三方APP, 第三方APP需在 -[DTOpenAPIDelegate onReq:] 处理钉钉的响应结果.
 * @param req 封装请求的对象. 支持的类型: DTSendMessageToDingTalkReq.
 * @param completion 发送回调，success YES 请求发送成功. NO 请求发送失败.
 */
+ (void)sendReq:(DTBaseReq *)req completion:(void (^)(BOOL success))completion;
