...
namespace Demo.Douyin.Game
{
    ...
    /// 抖音授权相关接口
    /// </summary>
    public class DemoStandardDouyin : MonoBehaviour
    {
        //【以下代码，外部方法】------------------------------------------------------------------
        ...
        private void AuthorizeSuccess(string token, string openid)
        {
            ...
            ShowToastAndPrint($"抖音授权成功，token = {token}, openid = {openid}");
        }

        private void AuthorizeFailed(BaseErrorEntity<DouyinAuthorizeErrorEnum> entity)
        {
            // TODO 请处理抖音授权失败后的游戏逻辑
            var msg = $"抖音授权失败，code = {entity.ErrorEnum}, msg = ${entity.Message}";
            ShowToastAndPrint(msg);
        }
        ...
        private async void RequestOAuth(string authCode)
        {
            ...
            // 将结果需回调给RequestOAuthSuccess(token, openid)或RequestOAuthFailed(errorEntity)
            
            var clientKey = OSDKIntegration.AndroidClientKey;
#if UNITY_IOS
            clientKey = OSDKIntegration.iOSClientKey;
#endif
            var clientSecret = GetDemoClientSecret(clientKey);
            if (string.IsNullOrWhiteSpace(clientSecret))
            {
                RequestOAuthFailed(new BaseErrorEntity<DouyinAuthorizeErrorEnum>()
                {
                    ErrorEnum = DouyinAuthorizeErrorEnum.OTHERS,
                    Message = $"抖音授权失败, 授权Client Secret为空"
                });
                throw new Exception("授权Client Secret不能为空，请填写");
            }

            // 抖音接口文档 https://developer.open-douyin.com/docs/resource/zh-CN/dop/develop/openapi/account-permission/get-access-token
        ...
        private void AuthResponseCallback(AuthResponse response)
        {
            if (response.ErrorEnum == DouyinAuthorizeErrorEnum.SUCCESS)
            {
                ...
                {
                    RequestOAuthSuccess("mock_token", "mock_openid");
                    return;
                }      
#endif
        ...
        // 仅Demo使用函数
        public static DemoStandardDouyin Instance => DemoGameObjectUtils.GetOrCreateGameObject<DemoStandardDouyin>();
        
        public static void ShowToastAndPrint(string message)
        {
            message = "DouYin Auth " + message;
            DemoLog.D(message);
            ToastManager.Instance.ShowToast(message);
        }

        private string GetDemoClientSecret(string clientkey) {
            if (clientkey == "awqvwg1wusqhsp89")
            {
                return "26833f1fe4345a0b77153fea92f7f298";
            }
            if (clientkey == "awxxllllhja6eit8")
            {
                return "7444844f57aa63f3082d458d2abe0f3a";
            }
            if (clientkey == "awfuz1noudnjemg7") 
            {
                return "2dca85dfda1f10ee55d2f813fcf56ef1";
            }
            if (clientkey == "awo8y01wevy43qpj")
            {
                return "1bca525a000ba9445c042addfd51dd29";
            }
            if (clientkey == "awna6vx7ggxfij8p")
            {
                return "75ca13cd5462061410c6c43a50ba6741";
            }
            if (clientkey == "awcw0enwjlvwyc8c")
            {
                return "71217f4990982556a2dfc08c09514266";
            }
            ...
            if (clientkey == "aw6lav9pq031veh4")
            {
                return "5c186df5fdbb8d29d24392dcfc5528a9";
            }
            if (clientkey == "awicuvtr1f8qt2qe")
            {
                return "bbfd42fcf1f7269283cfd60bf14ce18a";
            }
            if (clientkey == "aw44ttfxz1kbt9ws")
            {
                return "7a10d17231df7a74fe82777b7ea1e451";
            }
            return "";
