...
namespace ET
{
    public static class DropHelper
    {
		...
		//传入掉落ID，生成数据
		public static List<int> TreasureDropItmeShow(int dropID, int showNum) {
			...
			for (int i = 0; i < showNum; i++){
				...
				try
				{
					int randNum = RandomHelper.NextInt(0, (showIDList.Count - 1));
					Log.Info("randNum = " + randNum + "showNum = " + showNum + "showIDList.Count =" + showIDList.Count);
					showID.Add(showIDList[randNum]);
				}
				catch (Exception ex) {
