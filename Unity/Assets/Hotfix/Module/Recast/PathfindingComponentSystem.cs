...
namespace ET
{
    public static class PathfindingComponentSystem
    {
        ...
        public static Vector3 RecastFindNearestPoint(this PathfindingComponent self, Vector3 pos)
        {
            ...
            if (ret == 0)
            {
                throw new Exception($"RecastFindNearestPoint fail, 可能是位置配置有问题: sceneName:{self.DomainScene().Name} {pos} {self.Name} {self.GetParent<Unit>().Id} {self.GetParent<Unit>().ConfigId} {self.EndPos.ArrayToString()}");
            }
