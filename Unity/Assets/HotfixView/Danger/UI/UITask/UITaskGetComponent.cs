...
namespace ET
{
    ...
    public static class UITaskGetComponentSystem
    {
        ...
        public static void InitData(this UITaskGetComponent self, int npcID)
        {
            ...
            //显示Npc对话   
            self.Lab_NpcSpeak.GetComponent<Text>().text = "   " + npcConfig.GetSpeakText();
            self.Lab_NpcName.GetComponent<Text>().text = npcConfig.GetName();
            ...
            switch (npcConfig.NpcType)
            {
                ...
                case 4: //魔能老人
                    ...
                    //获取
                    self.Obj_Lab_MoNnengHint.GetComponent<Text>().text = ItemConfigCategory.Instance.Get(costItemID).GetItemName() + "  " + itemNum + "/" + 5;
                    break;
                case 5://补偿大师
                    self.TaskFubenList.SetActive(true);
                ...
                case 6: //节日使者
                    ...
                    if (activityId == 0)
                    {
                        ...
                        self.Lab_NpcSpeak.GetComponent<Text>().text = string.Format(GameSettingLanguge.LoadLocalization("{0} 下次领取时间:{1}月{2}日 {3}"), speek, riqi[0], riqi[1], activityConfig.GetPar_4());
                    }
                    break;
                case 8:  //经验兑换
        ...
        public static void OnButtonFragmentHuan(this UITaskGetComponent self)
        {
            ...
            PopupTipHelp.OpenPopupTip( self.ZoneScene(), GameSettingLanguge.LoadLocalization("碎片兑换"), string.Format(GameSettingLanguge.LoadLocalization("是否消耗一个神兽碎片兑换一个{0}"), itemConfig.GetItemName()), ()=>
            {
        ...
        public static void OnClickFubenItem(this UITaskGetComponent self,  int npcType, int configId)
        {
            ...
            if (npcType == 2)
            {
                ...
                string desStr = string.Format(GameSettingLanguge.LoadLocalization("是否进入{0}?"), sceneConfig.GetName());
