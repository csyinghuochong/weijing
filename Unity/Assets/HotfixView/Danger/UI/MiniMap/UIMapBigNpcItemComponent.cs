...
namespace ET
{
    ...
    public static class UIMapBigNpcItemComponentSystem
    {
        ...
        public static void SetClickHandler(this UIMapBigNpcItemComponent self, int unittype, int npcId, Action<int, int> action)
        {
            ...
            {
                self.TextName.GetComponent<Text>().text = NpcConfigCategory.Instance.Get(npcId).GetName();
            }
            else
            {
                self.TextName.GetComponent<Text>().text = MonsterConfigCategory.Instance.Get(npcId).GetMonsterName();
            }
        }
