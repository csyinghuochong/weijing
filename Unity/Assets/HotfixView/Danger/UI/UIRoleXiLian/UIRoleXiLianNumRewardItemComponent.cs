...
namespace ET
{
    public class UIRoleXiLianNumRewardItemComponent: Entity, IAwake<GameObject>
    {
    ...
    public class UIXiLianNumRewardItemComponentAwakeSystem: AwakeSystem<UIRoleXiLianNumRewardItemComponent, GameObject>
    {
        public override void Awake(UIRoleXiLianNumRewardItemComponent self, GameObject gameObject)
        {
    ...
    public static class UIRoleXiLianNumRewardItemComponentSystem
    {
        public static async ETTask OnBtn_Reward(this UIRoleXiLianNumRewardItemComponent self)
        {
            ...
            if (self.ZoneScene().GetComponent<BagComponent>().GetLeftSpace() < ConfigHelper.ItemXiLianNumReward[self.RewardKey].Split('@').Length - 1)
            {
                FloatTipManager.Instance.ShowFloatTip("背包空间不足！");
                return;
            }

            C2M_ItemXiLianNumReward request = new C2M_ItemXiLianNumReward() { RewardId = self.RewardKey };
            M2C_ItemXiLianNumReward response =
                    (M2C_ItemXiLianNumReward)await self.ZoneScene().GetComponent<SessionComponent>().Session.Call(request);
        ...
        public static void UpdateButton(this UIRoleXiLianNumRewardItemComponent self)
        {
            UserInfoComponent userInfoComponent = self.ZoneScene().GetComponent<UserInfoComponent>();
            bool received = userInfoComponent.UserInfo.ItemXiLianNumRewardIds.Contains(self.RewardKey);
        ...
        public static void OnUpdateUI(this UIRoleXiLianNumRewardItemComponent self, int key)
        {
            self.RewardKey = key;
            string[] reward = ConfigHelper.ItemXiLianNumReward[key].Split('$');
