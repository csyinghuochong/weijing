    ...
    public static class UIPetMiningRecordComponentSystem
    {
        ...
        public static async ETTask  OnInitUI(this UIPetMiningRecordComponent self)
        {
            ...
            for (int i = 0; i < response.PetMingRecords.Count; i++)
            {
                ...
                string content = string.Format(GameSettingLanguge.LoadLocalization("玩家 {0} {1} 占领了你的{2}"), response.PetMingRecords[i].WinPlayer, TimeInfo.Instance.ToDateTime(petMingRecord.Time), GameSettingLanguge.LoadLocalization(mineBattleConfig.Name));
