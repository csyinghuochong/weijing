            ...
            for (int i = 0; i < response.JiaYuanRecordList.Count; i++)
            {
                ...
                switch (jiaYuanRecord.OperateType)
                {
                    ...
                    case JiaYuanOperateType.GatherPlant:
                        tip += string.Format(GameSettingLanguge.LoadLocalization(" 在你的家园拾取了<color=#9CA606> {0}</color>"), JiaYuanFarmConfigCategory.Instance.Get(jiaYuanRecord.OperateId).GetName());
                        break;
                    case JiaYuanOperateType.GatherPasture:
                        tip += string.Format(GameSettingLanguge.LoadLocalization(" 在你的家园拾取了<color=#9CA606> {0}</color>"), JiaYuanPastureConfigCategory.Instance.Get(jiaYuanRecord.OperateId).GetName());
                        break;
                    case JiaYuanOperateType.Pick:
                        tip += string.Format(GameSettingLanguge.LoadLocalization(" 在你的家园清理了<color=#9CA606> {0}</color>"), MonsterConfigCategory.Instance.Get(jiaYuanRecord.OperateId).GetMonsterName());
                        break;
                    default:
                        break;
                }
