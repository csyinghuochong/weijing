...
namespace ET
{
    ...
    public static class UIXiuLianComponentSystem
    {
        ...
        public static void OnUpdateUI(this UIXiuLianComponent self)
        {
            ...
            if (leftExpCD > 0)
            {
                self.Text_ExpCD.GetComponent<Text>().text = GameSettingLanguge.LoadLocalization("修炼时间倒计时:") + UICommonHelper.ShowLeftTime(leftExpCD, GameSettingLanguge.Language);
            }
            else
            {
            ...
            if (leftCoinCD > 0)
            {
                self.Text_CoinCD.GetComponent<Text>().text = GameSettingLanguge.LoadLocalization("修炼时间倒计时:") + UICommonHelper.ShowLeftTime(leftCoinCD, GameSettingLanguge.Language);
            } 
            else
            {
