...
namespace ET
{
    ...
    public static class UIPetEggChouKaRewardItemComponentSystem
    {
        public static async ETTask OnBtn_Reward(this UIPetEggChouKaRewardItemComponent self)
        {
            ...
            if (self.ZoneScene().GetComponent<BagComponent>().GetBagLeftCell() < ConfigHelper.PetExploreReward[self.RewardKey].Split('@').Length - 1)
            {
        ...
        public static void OnUpdateUI(this UIPetEggChouKaRewardItemComponent self, int key)
        {
            ...
            self.TextNeedTimes.GetComponent<Text>().text = string.Format(GameSettingLanguge.LoadLocalization("探索次数达到{0}次"), key);
