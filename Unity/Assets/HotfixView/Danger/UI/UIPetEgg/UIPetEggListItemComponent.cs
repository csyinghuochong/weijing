...
namespace ET
{
    ...
    public static class UIPetEggListItemComponentSystem
    {
        ...
        public static void OnTimer(this UIPetEggListItemComponent self)
        {
            ...
            self.Text_Time.GetComponent<Text>().text = GameSettingLanguge.LoadLocalization("剩余时间:") + UICommonHelper.ShowLeftTime(timeNow, GameSettingLanguge.Language);
            if (timeNow <= 0)
            {
                self.SetFuHuaEnd();
                TimerComponent.Instance.Remove(ref self.Timer);
        ...
        public static void SetEggNoFuhua(this UIPetEggListItemComponent self)
        {
            ...
            self.Text_Time.GetComponent<Text>().text = GameSettingLanguge.LoadLocalization("孵化时间:") + UICommonHelper.ShowLeftTime(timeNow*1000, GameSettingLanguge.Language);
            self.ButtonOpen.SetActive(false);
            self.ButtonGet.SetActive(false);
        }
