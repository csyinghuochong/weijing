...
namespace ET
{
    ...
    public static class UIFenXiangSetComponentSystem
    {
        ...
        public static void FenXiangByType(this UIFenXiangSetComponent self, int shareType)
        {
            ...
            content.SetTitle(fenXiangContent.FenXiang_Title);
            //分享文本
            content.SetText(fenXiangContent.FenXiang_Text);
            //分享网络图片，新浪微博分享网络图片需要通过审核后申请高级写入接口，否则请注释掉测试新浪微博
            content.SetImageUrl(fenXiangContent.FenXiang_ImageUrl);
            // titleUrl是标题的网络链接，仅在Linked-in,QQ和QQ空间使用
            /////content.SetTitleUrl(fenXiangContent.FenXiang_ClickUrl);
            // imagePath是图片的本地路径，Linked-In以外的平台都支持此参数
            //content.SetImagePath("/sdcard/test.jpg");//确保SDcard下面存在此张图片
            // url仅在微信（包括好友和朋友圈）中使用
            ///////content.SetUrl(fenXiangContent.FenXiang_ClickUrl);
            // site是分享此内容的网站名称，仅在QQ空间使用
            //////content.SetSite(fenXiangContent.FenXiang_Title);
            // siteUrl是分享此内容的网站地址，仅在QQ空间使用
            ///////content.SetSiteUrl(fenXiangContent.FenXiang_ClickUrl);
            //设置分享类型
