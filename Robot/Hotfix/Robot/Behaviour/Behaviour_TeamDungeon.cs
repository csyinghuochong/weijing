    ...
    public class Behaviour_TeamDungeon : BehaviourHandler
    {
        ...
        public override async ETTask Execute(BehaviourComponent aiComponent, AIConfig aiConfig, ETCancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                if (teamInfo != null)
                {
                    errorCode = await teamComponent.SendTeamApply(teamInfo.TeamId, teamInfo.SceneId, teamInfo.FubenType, teamInfo.PlayerList[0].PlayerLv, true);
                }
                if (errorCode != 0)
                {
                ...
                bool ret = await TimerComponent.Instance.WaitAsync(TimeHelper.Minute * 5, cancellationToken);
                if (ret)
                {
                    aiComponent.Exit( "没有进入队伍");
                    return;
                }
                else
                {
                    Log.Debug("Behaviour_TeamDungeon: Change");
                    return;
                }
            }
        }
